...
STATIC mp_obj_t mod_trezorcrypto_slip39_word_completion_mask(mp_obj_t _prefix) {
  ...
  if (prefix < 1 || prefix > 9999) {
    mp_raise_ValueError(
        MP_ERROR_TEXT("Invalid button prefix (range between 1 and 9999 is allowed)"));
  }
...
mod_trezorcrypto_slip39_button_sequence_to_word(mp_obj_t _prefix) {
  ...
  if (word == NULL) {
    mp_raise_ValueError(MP_ERROR_TEXT("Invalid button prefix"));
  }
...
STATIC mp_obj_t mod_trezorcrypto_slip39_get_word(mp_obj_t _index) {
  ...
  if (word == NULL) {
    mp_raise_ValueError(
      MP_ERROR_TEXT("Invalid wordlist index (range between 0 and 1024 is allowed)"));
  }
