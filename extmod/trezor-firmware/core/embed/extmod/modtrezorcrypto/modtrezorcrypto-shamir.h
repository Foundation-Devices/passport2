...
mp_obj_t mod_trezorcrypto_shamir_interpolate(mp_obj_t shares, mp_obj_t x) {
  ...
  for (int i = 0; i < share_count; ++i) {
    ...
    if (value_len == 0) {
      ...
      if (value_len > SHAMIR_MAX_LEN) {
        mp_raise_ValueError(MP_ERROR_TEXT("Share value exceeds maximum supported length."));
      }
    }
    if (value.len != value_len) {
      mp_raise_ValueError(MP_ERROR_TEXT("All shares must have the same length."));
    }
  ...
  if (shamir_interpolate((uint8_t *)vstr.buf, x_uint8, share_indices,
    ...
    vstr_clear(&vstr);
    mp_raise_ValueError(MP_ERROR_TEXT("Share indices must be pairwise distinct."));
  }
