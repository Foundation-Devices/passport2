...
from constants import NUM_BACKUP_PASSWORD_WORDS, TOTAL_BACKUP_CODE_DIGITS
from flows import FilePickerFlow, Flow
from pages import (BackupCodePage, ErrorPage, InsertMicroSDPage, LongSuccessPage,
                   PredictiveTextInputPage, QuestionPage, RecoveryModeChooserPage, SuccessPage)
...
class VerifyBackupFlow(Flow):
    ...
    async def select_recovery_mode(self):
        ...
        result = await RecoveryModeChooserPage(card_header={'title': 'Verify Backup'}).show()
        if result is None:
            self.clear_credentials()
            self.back()
        elif result == RecoveryMode.BACKUP_CODE_20_DIGITS:
    ...
    async def enter_backup_password(self):
        ...
        (words, self.backup_password_prefixes, _) = result
        if words is None:
            cancel = await QuestionPage(text='Cancel password entry? All progress will be lost.').show()
            if cancel:
                self.clear_credentials()
                self.back()
        else:
            self.backup_password_words = words
            self.decryption_password = ' '.join(words)
